<template>
  <div class="addent">
    <el-dialog :title="$parent.isedit?'编辑企业':'新增企业'" :visible.sync="dialogFormVisible">
      <el-form :model="form" :rules="rules" ref="formname">
        <el-form-item label="企业编号" prop="eid" :label-width="formLabelWidth" class="entname">
  ...
  methods: {
    onsubmit() {
      this.$refs.formname.validate(valid => {
        if (valid) {
          if (this.$parent.idedit) {
              //执行编辑逻辑
            apienteredit(this.form).then(res => {
